...
# get raster 
file<-list.files(dir, full.name=TRUE) %>%
str_subset('.tif$')

nightlights_raster<-raster(file)

# match shapefile and raster coordinate system
japan_shp<-st_transform(japan_shp,st_crs(nightlights_raster))
...
geom_text(aes(x=jpn_bbox[1]+.03*x_diff,y=jpn_bbox[4]-.18*y_diff, label=subtitle), family="Advent Pro", color="#9C9C9C", size=5)+
geom_segment(aes(x=jpn_bbox[1]-.15*x_diff, y=jpn_bbox[4]-.28*y_diff, xend=jpn_bbox[1]+.27*x_diff,yend=jpn_bbox[4]-.28*y_diff), linetype="dashed", size=.3, color="#9C9C9C", arrow=arrow(type="closed",length=unit(.013,"npc")))+
geom_text(aes(x=jpn_bbox[1]+.04*x_diff,y=jpn_bbox[4]-.29*y_diff,label="Increasing light emission"),family="Advent Pro", color="#9C9C9C",size=3.5)+
...
ggsave(plot = p, filename = "space_test.png", width=8, height=6, units="in", dpi=300, device=ragg::agg_png, bg="#0E0E0E")
